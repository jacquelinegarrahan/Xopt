...
def write_txt_population(population, filename):
    ...
    if hasattr(population[0], "objective_labels"):
        header = (
            population[0].objective_labels
            + ["constraints_satisfied"]
            + population[0].variable_labels
            + reference_keys
        )
        for x in header:
            f.write(x)
...
def read_txt_population(population, filename):
    ...
    for ix_label in range(len(header)):
        ...
        if len(ix) == 1:
            ...
            print(
                "Error: Multiple (" + str(len(ix)) + ") variable labels for: " + label
            )
...
def nsga2_toolbox(
    ...
    if n_constraints == 0:
        ...
        creator.create(
            "MyFitness",
            fitness_with_constraints.FitnessWithConstraints,
            weights=weights,
...
def evaluate_population(toolbox, population, archive_h5=None):
    """
...
def evaluate_all(individuals, toolbox, archive_filePath=None):
    """
...
def main(
    ...
    ]  # Reevaluate all if not ind.fitness.valid]
    vecs = [
        list(ind) for ind in population
    ]  # Fixes a problem with mpi4py and with starting from checkpoints
    ...
    if not skip_checkpoint_eval:
        ...
        vprint(
            str(len(invalid_ind))
            + " fitness calculations for initial generation...DONE"
        )
    ...
    for gen in range(start_gen, NGEN):
        ...
        vprint(
            str(len(invalid_ind))
            + " fitness calculations for generation "
            + str(gen)
            + " ..."
        )
        tstart = time.time()
        ...
        if do_history:
            pickle.dump(
                ...
                open(os.path.join(output_dir, "history.pkl"), "wb"),
            )
