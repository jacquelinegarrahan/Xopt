...
class ContinuousProblem(Problem):
    ...
    def __init__(
        ...
        if DEPRECATED_PYMOO:
            ...
            ), "Must have elementwise_evaluation for continuous evaluation "
    ...
    def submit_dummies(self, n):
        ...
        return [
            self.executor.submit(
                dummy_f, n_var=self.n_var, n_obj=self.n_obj, n_constr=self.n_constr
            )
            for i in range(n)
        ]
    ...
    def _evaluate_elementwise(self, X, calc_gradient, out, *args, **kwargs):
        ...
        while not done:
            for ix, fut in enumerate(self.futures):
                ...
                results.append(res)
                self.futures[ix] = self.executor.submit(
                    self.f, X[i0]
                )  # send off new work
...
def cmoo_evaluate(
    ...
    try:
        ...
        if vocs:
            ...
            inputs0 = (
                inputs.copy()
            )  # Make a copy, because the evaluate function might modify the inputs.
        ...
        obj_eval = -np.array(vocs_tools.weight_list(vocs["objectives"])) * np.array(
            obj_eval
        )
...
def cnsga2(
    ...
    toolbox = Toolbox()
    toolbox.register(
        "f", cmoo_evaluate, labeled_evaluate_f=evaluate_f, vocs=vocs, verbose=verbose
    )
    ...
    res = minimize(
        problem, algorithm, ("n_gen", max_generations), seed=seed, verbose=False
    )
