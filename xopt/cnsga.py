...
def cnsga_evaluate(
    vec, evaluate_f=None, vocs=None, include_inputs_and_outputs=True, verbose=True
):
    """
...
def pop_init(vocs, data):
    ...
    if n_extra > 0:
        logger.warning(
            f"Warning: trimming {n_extra} from initial population to make a multiple of 4."
        )
        vecs = vecs[:-n_extra]
        ...
        for ix in range(len(futures)):
            ...
            if fut.done():
                ...
                if len(new_vecs) == 0:
                    ...
                    logger.info(
                        f"Generation {generation} completed in {dt/60:0.5f} minutes"
                    )
                    generation += 1
