...
def parse_algorithm_config(algorithm_config: Dict):
    ...
    algorithm_config = check_and_fill_defaults(algorithm_config, ALGORITHM_DEFAULTS)
    ...
    all_default_options = {**algorithm_default_options, **generator_default_options}
    ...
    generator_kwargs = {
        k: algorithm_config["options"][k] for k in generator_default_options.keys()
    }

    algorithm_kwargs = {
        k: algorithm_config["options"][k] for k in algorithm_default_options.keys()
    }

    return algorithm_config, algorithm_kwargs, generator_config, generator_kwargs


def parse_evaluator_config(evaluator_config: Dict):
    # check high level options are correct for evaluate
    evaluator_config = check_and_fill_defaults(evaluator_config, EVALUATE_DEFAULTS)
    ...
    evaluator_config["options"] = check_and_fill_defaults(
        evaluator_config["options"] or {}, evaluate_function_defaults
    )
    ...
    if isinstance(executor, str):
        if executor == "ThreadPoolExecutor":
            ...
            raise RuntimeError(
                f"specified executor via string {executor} does not " f"exit"
            )
    ...
    if n_required_args != 1:
        raise ValueError(
            f"function has {n_required_args}, but should have " f"exactly one. "
        )
